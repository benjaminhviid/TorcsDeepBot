...
public class DrunkKitt extends Controller {
	...
	float maxSpeedDist = 25;
	final float maxSpeed = 81;
	final float sin5 = (float) 0.08716;
	final float cos5 = (float) 0.99619;

	/* Steering constants */
	final float steerLock = (float) 0.366519;
	final float steerSensitivityOffset = (float) 170.0;
	...
	private static String path = "C:\\DataLogger";
	private static String newFolder = "\\training_data";
	...
	private float getSteer(SensorModel sensors) {
		...
		float targetAngle = (float) (sensors.getAngleToTrackAxis() - (sensors.getTrackPosition()+0.5f) * 0.5);
	...
	private float getDrunk(float val) {
		Random rand = new Random();
		float drunkVal =  (rand.nextInt(50)+1)/100 + 0.74f;
	...
	private float getAccel(SensorModel sensors) {
		...
		if (sensors.getTrackPosition() < 1 && sensors.getTrackPosition() > -1) {
			...
			if (sensorsensor > getDrunk(maxSpeedDist) || (sensorsensor >= rxSensor && sensorsensor >= sxSensor))
				...
				if (rxSensor > sxSensor) {
					...
					// close it is
					targetSpeed = maxSpeed * (sensorsensor * sinAngle / getDrunk(maxSpeedDist));
				}
			...
			return (float) 0.3; // when out of track returns a moderate
	...
	public Action control(SensorModel sensors) {
		...
		Double normalizedTrackPos;

		if (sensors.getTrackPosition() > 1.0)
			normalizedTrackPos = 1.0;
		else if (sensors.getTrackPosition() < -1.0)
			normalizedTrackPos = -1.0;
		else
			normalizedTrackPos = sensors.getTrackPosition();

		speed  = sensors.getSpeed();
		angle = sensors.getAngleToTrackAxis();
		dist_RL = Math.abs(normalizedTrackPos);
		dist_RR = Math.abs(normalizedTrackPos - 1);
		...
		if (stuck > stuckTime) {
			/*
			 * set gear and sterring command assuming car is pointing in a
